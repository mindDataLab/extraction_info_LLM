    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt < max_retries:
                ...
                history.append({"role": "user", "content": "Votre réponse précédente n'était pas un JSON valide. Veuillez corriger le format et ne renvoyer que le JSON corrigé, sans texte supplémentaire."})
            else:
...
def process_batch(user_id, system_prompt):
    ...
    for filename in files_to_process:
        ...
        if extracted_data:
            ...
            if success:
                print(f"Données sauvegardées/mises à jour dans la base de données pour l'utilisateur {user_id}.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Script de traitement par lots pour l'extraction de données.")
    parser.add_argument("--user", type=str, required=True, help="Nom d'utilisateur pour associer les extractions.")
    args = parser.parse_args()

    # Vérifier si l'utilisateur existe
    user = database.get_user(args.user)
    if not user:
        print(f"ERREUR: L'utilisateur '{args.user}' n'existe pas. Veuillez d'abord le créer via l'application web.")
    else:
        # Déterminer le prompt système à utiliser
        if user['custom_system_prompt']:
            system_prompt_to_use = user['custom_system_prompt']
            print(f"Utilisation du prompt personnalisé pour l'utilisateur '{args.user}'.")
        else:
        ...
        if system_prompt_to_use:
            ...
            print("ERREUR: Le prompt système est vide. Impossible de lancer le traitement.")